...
class L3_IP:
    ...
    def coonectLayers(self, underLayers, upperLayer, arpLayers):
        self.underLayers = underLayers
        self.upperLayer = upperLayer
        self.arpLayers = arpLayers

    def coonectTable(self, fowardingtable):
        self.fowardingtable = fowardingtable

    def receive(self, ppayload):
        self.extractHeader(ppayload)

        index = self.fowardingtable.search(self.pdst)
        if index != None:
            self.send(index)

    def send(self, index):
        print('Tuple: ', self.fowardingtable.get_tuple(index))
    ...
    def generatePayload(self):
        return self._pverlen + self._pservice + self._ptotal + self._pid + self._pflag_and_offset + self._pttl \
                + self._ptype + self._pcheck_sum + self._psrc + self._pdst + self._data